...
def evaluate_minority(model, X_test, y_test, config, filename=None, title="", log=False):    
    ...
    predictions, probabilities = model_utils.perform_inference(model, X_test, config["batch_size"], config["device"], is_bert=config["model_name"].lower() == "bert")
...
def get_shap_values(model, data, config):
    def predict(X):
        ...
        X.to(config["device"])
        pred, _ = model_utils.perform_inference(model, X, config["batch_size"], config["device"], is_bert=config["model_name"].lower() == "bert")
